...
export default function ServicesSection() {
  return (
    <section id="services" className="border-b-2 border-[#4C211B] bg-white py-16 sm:py-24 px-4 sm:px-6">
      <div className="max-w-5xl mx-auto flex flex-col gap-10 sm:gap-12">
        <div className="flex flex-col gap-3">
          ...
          <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl font-medium tracking-tight">What I can help you build.</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6">
          {SERVICES.map(s => (
            <div
              key={s.num}
              className={`${s.bg} p-6 sm:p-8 rounded-lg border-2 border-[#4C211B] shadow-[6px_6px_0px_#4C211B] hover:-translate-y-1 transition-transform flex flex-col gap-3 sm:gap-4`}
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white rounded-full border-2 border-[#4C211B] flex items-center justify-center text-base sm:text-xl font-medium shadow-[2px_2px_0px_#4C211B]">
                {s.num}
              </div>
              <h3 className="text-xl sm:text-2xl font-medium tracking-tight">{s.title}</h3>
              <p className="text-sm sm:text-base font-semibold text-[#C12A15]">{s.price}</p>
              <p className="text-base sm:text-lg text-[#4C211B]/70 leading-relaxed">{s.desc}</p>
